                ...
                $("<ul>", {
                    ...
                    $("<li>").append(
                        $("<a>", {
                            tabindex: "-1",
                            class: "subError",
                            err: 'accuracy:lettersubstitution',
                            html: 'Substituição de letras'
                        })
                    ),
                    $("<li>").append(
                        $("<a>", {
                ...
                $("<ul>", {
                    ...
                    $("<li>").append(
                        $("<a>", {
                            tabindex: "-1",
                            class: "subError",
                            err: 'fluidity:spontaneousrectification',
                            html: 'Retificação espontânea'
                        })
                    )
                )
            )
...
window.getQuestionVoice = function(question) {
  //console.log(question);
  //console.log(question._id);
    var self = this;
    self.bd2 = 'aprender_questions';
    self.site = 'https://letrinhas.ipt.pt'; //process.env.COUCHDB;

    var $soundDiv = $('<div>');
    $($soundDiv)
        .append(' <button class="btn btn-default btn-xs audioDownload"  onClick=setSyncr(' + question._id + ') id="toAppend' + question._id +'">Ouvir Professor</button>'
          )

    return $soundDiv;
};

function testa(question) {
    //console.log('entrou');
...
window.getTextPreview = function(question) {
    try {
        ...
        var $soundDiv = getQuestionVoice(question);
...
window.setSyncr = function(question) {
    //var audio1 = Popcorn("#teacherVoice" + question._id);
    //console.log('chegou');
    try{


      var quest = new Question({
          id: $(question).get(0).id
      });

      quest.fetch(function() {
        var question = quest.attributes;
          ...
          $('.word').click(function() {
              var audio = $('#teacherVoice' + question._id);
...
window.setMultimediaPreview = function(question, div) {
    ...
    try {
        ...
        switch (question.content.questionType) {
            case "audio":
                //Adiciona o som
                $contentDiv.append(getQuestionVoice(question));
                break;
            case "image":
                //Adiciona a imagem
                $contentDiv.append($('<div>', {
            ...
            case "text":
                ...
                $contentDiv.append($('<label>', {
                    class: 'col-md-12 questBox centered',
...
window.getCorrectionPreview = function(question) {
    ...
    if(question.info!==null){
      ...
      alertMsg($("body"), "Ocorreu um erro irrecuperável, prossiga com a remoçao desta resolução!");
      setTimeout(function() {
          window.history.back();
          $('#infoPop').popover("hide");
      }, 1500);
...
window.setCorrecInterpretationPreview = function(question) {
    ...
    var $soundDiv = getQuestionVoice(info);
    var words = $();
    var $text = $('<div>', {
        class: 'questBox col-md-12'
    });
    $contentDiv.append($text);
    ...
    $correctDiv.append(
        ...
        $('<label>', {
            class: "col-md-7 row",
            html: 'Total de opções'
        }),
        $('<label>', {
            class: "col-md-5 row",
            html: info.content.sid.length
        }),
        $('<label>', {
            class: "col-md-7 row",
            html: 'Opções correctas'
        }),
        $('<label>', {
            class: "col-md-5 row rightAnswer",
            html: nRights
        }),
        $('<label>', {
            class: "col-md-7 row",
        ...
        $('<label>', {
            class: "col-md-5 row wrongAnswer",
            html: resol.answer.solution.length - nRights
        }),
        $('<label>', {
            class: "col-md-7 row",
            html: 'Opções não encontradas'
        }),
        $('<label>', {
            class: "col-md-5 row markedWord",
            html: info.content.sid.length - nRights
        }),
        $('<div>', {
            class: "col-md-12 row"
        }).append(
...
window.setCorrecMultimediaPreview = function(question) {
    ...
    if(resol!==null){
      ...
      switch (info.content.questionType) {
          case "audio":
              //Adiciona o som
              $contentDiv.append(getQuestionVoice(info));
              break;
          case "image":
              //Adiciona a imagem
              $contentDiv.append($('<div>', {
      ...
      $correctDiv.append(
          ...
          $('<label>', {
              class: "col-md-7 row",
              html: 'Resposta do aluno'
          }),
          $('<label>', {
              class: "col-md-5 row",
              html: (0 == resol.answer.solution ? "Correcta" : "Incorrecta")
          }),
          $('<div>', {
              class: "col-md-12 row"
          }).append(
              $('<input>', {
                  ...
                  readonly: "readonly",
                  dif: question.dif,
                  value: (0 == resol.answer.solution ? "100" : "0")
              }),
              $('<span>', {
                  class: "glyphicon"
              }).append(
...
window.setCorrecListPreview = function(question) {
    ...
    x.setAttribute("src", "https://letrinhas.ipt.pt" + "/file/" + "aprender_resolutions" + "/" + resol._id + "/record.m4a");
    ...
    $contentDiv.append(
        $soundDiv, $('<label>', {
            class: "col-md-4 audioLbl",
            html: 'Gravação do aluno'
        }),
        $('<audio>', {
            class: "col-md-10",
            "controls": "controls",
        ...
        .append(
            $('<source>', {
                src: "https://letrinhas.ipt.pt" + "/file/" + "aprender_resolutions" + "/" + resol._id + "/record.m4a",
                type: "audio/mp4"
            })
        )
    );
    ...
    $correctDiv.append(
        ...
        $('<label>', {
            class: "col-md-7 row",
            html: 'Palavras por minuto:'
        }),
        $('<label>', {
            class: "col-md-5 row",
            id: "wordsMin",
            html: parseInt(nWords / (studentDuration / 60))
        }),
        $('<label>', {
            class: "col-md-7 row",
            html: 'Total de erros:'
                ...
                $modalBody.append(
                    $('<div>', {
                        ...
                        $('<a>', {
                            ...
                            $('<p>', {
                                class: 'pictureLabel',
                                html: name[0] + " " + name[name.length - 1]
                            })
                        ).click(function() {
                            //actualiza a pagina
                ...
                $("<div>", {
                    ...
                    $("<div>", {
                        ...
                        $("<div>", {
                            ...
                            $("<h3>", {
                                class: "modal-title",
                                text: classData.year + "º " + classData.name
                            })
                        )
                        // MODAl HEATHER
                    ).append(

window.TestsView = Backbone.View.extend({
    events: {
        ...
        'click .audioDownload' : 'downloadSound'

    },

    downloadSound: function(e) {
        var self = this;

         var iad = $(e.target).closest('.audioDownload').attr('id').toString();
         var concatID = iad.substr(8);

          $('<audio>', {
                  class: "col-md-10 audioToPlay ",
                  "controls": "controls",
                  id: "teacherVoice" + concatID

              }).append(
                  $('<source>', {
                      src: 'https://letrinhas.ipt.pt' + "/file/" + self.bd2 + "/" + concatID + "/voice.mp3",
                      type: "audio/mpeg"

                  })
              ).insertAfter('#toAppend'+concatID);
            ...
            $.each(test.questions, function (testKey, quest) {
                ...
                question.fetch(function () {
                    ...
                    $("#mvCrsl" + testID + " .carousel-inner").append($('<div>', {
                        class: 'item sound'+testKey, html: quest.title, id: 'questionsPreview' + (testKey + '').concat(testID)
                    }));
    ...
    enchePreview: function (question, i) {
        ...
        $("#questionsPreview" + i)
            .append(
                ...
                $('<div>', {
                    class: "col-md-12 sound"+question._id+"", id: "questionBox" + i
                })
            )
        ;
    ...
    deleteTest: function (e) {
        ...
        // /test.userID=window.sessionStorage.user;
    ...
    initialize: function () {
        ...
        this.bd2 = 'aprender_questions';
        this.site = 'http://letrinhas.ipt.pt:85';//process.env.COUCHDB;
        getStudents();
        getTypes();

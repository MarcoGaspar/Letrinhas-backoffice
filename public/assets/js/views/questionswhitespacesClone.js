window.QuestionsWhiteSpacesClone = Backbone.View.extend({
    events: {
    ...
    afterRender: function() {
        ...
        $.each(self.data.content.sid, function(iList, list) {
            Sids = Sids.concat(list.sids);
        })

        //Separa o texto em paragrafos
        var $paragraph = self.data.content.text.split(/\r|\n/);
        var words = $();
        var ponctuation = ['.', '!', '?', '-', ';', ':'];
        var nWords = 0;
        //por cada paragrafo adiciona a palavra a lista, e a new line

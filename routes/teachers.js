...
exports.getMyDataToEdit = function(req, res) {

    console.log('getting my data :    ' + req.params.bgBlue);

    var utilizador = req.user.name;

    db.get(utilizador, function(err, user) {
        if (err) {
            console.log("not found")
            return res.status(204).json({
                'result': 'nok',
        ...
        if (user.permissionLevel == 3) {
            ...
            db.get(req.params.teacher, function(err, body) {
                if (err) {
                    console.log("not found")
                    return res.status(204).json({
                        'result': 'nok',
                ...
                //gets schools and classes
                Schools.getClassesByTeacher(req.params.teacher, function(err, escolas) {
                    if (err) {
                        return res.status(500).json({
                            'result': 'nok',
                            'message': err
                    ...
                    body.schools = escolas;
                    Students.getByTeacher(req.params.teacher, function(err, students) {
                        if (err) {
                            return res.status(500).json({
                                'result': 'nok',
                                'message': err
                        ...
                        body.AuthAccess = user.permissionLevel;
                        //Return Result
                        res.json(body);
                    });
                })
        ...
        } else if (req.params.teacher == req.user.name) {
            db.get(req.params.teacher, function(err, body) {
                if (err) {
                    console.log("not found")
                    return res.status(204).json({
